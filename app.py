...
def summarize(tickers):
    ...
    start_date = end_date.replace(year=end_date.year - 11)  # use 11 years to ensure 10Y window has enough data
...
st.title("Historical & Periodic Volatility Scanner")

ticker_input = st.text_input("Enter a ticker symbol (e.g. SPY)", value="", placeholder="SPY").strip().upper()
...
if st.button("Run"):
    ...
    st.info("Type a ticker (like SPY or AAPL) and click Run.")
